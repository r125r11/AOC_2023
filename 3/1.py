...
def walkLine(l,s,y):
    ...
    if (s==0 and l[s].isdigit()) or (l[s-1] == '.' and l[s].isdigit()):walkRight(l,s,y)
    if l[s] == '.' and s<=len(l) and l[s+1].isdigit():walkRight(l,s+1,y)

def checkSurroundings(x, y):
    if y > 0:walkLine(f[y-1],x,y-1)
        
    l = f[y]
    if x-1 > 0 and l[x-1].isdigit(): walkLeft(l,x-1,y)
    if x+1 < len(l) and l[x+1].isdigit():walkRight(l,x+1,y)
